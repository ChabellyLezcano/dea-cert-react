...
export const Header = forwardRef<HTMLElement>(function Header(_props, ref) {
  ...
  return (
    <header
      ref={ref}
      className="fixed inset-x-0 top-0 z-20 border-b border-ink-100 bg-surface/90 backdrop-blur"
    >
      ...
      {open && (
        ...
        >
          ...
          <button
            type="button"
            onClick={() => signOut()}
            className="flex w-full items-center gap-2 rounded-lg border-t border-ink-100 px-3 py-2.5 text-left text-sm font-semibold text-ko-600 transition hover:bg-ko-100"
          >
